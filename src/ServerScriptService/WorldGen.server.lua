-- WorldGen.server.lua (Medieval upgrade v2)
-- Castle (towers + crenellations + interior stairs), river + bridge, village, roads, forest clusters, working gate.
...
local function mkTree(pos)
local trunk = mkPart("Trunk", Vector3.new(2, 12, 2), CFrame.new(pos + Vector3.new(0,6,0)), Color3.fromRGB(101,67,33), Enum.Material.Wood, true, Nature)
trunk.Shape = Enum.PartType.Cylinder; trunk.Orientation = Vector3.new(0,0,90)
...
leaves.CFrame = CFrame.new(pos + Vector3.new(0,14,0)); leaves.Parent = Nature
end
for i=1,70 do
local x = math.random(-380, 380); local z = math.random(-380, 380)
if math.abs(z - riverZ) > 40 then mkTree(Vector3.new(x, 1, z)) end
end
for i=1,40 do
local x = math.random(-380, 380); local z = math.random(-380, 380)
if math.abs(z - riverZ) > 40 then
mkPart("Rock"..i, Vector3.new(math.random(3,7), math.random(2,4), math.random(3,7)), CFrame.new(x, 2, z), Color3.fromRGB(110,110,110), Enum.Material.Rock, true, Nature)
end
end
