...
function Spawner.CloneAt(prefab: Instance, parent: Instance, cf: CFrame, scaleMin: number?, scaleMax: number?)
    ...
    local smin = scaleMin or 1.0
    local smax = scaleMax or 1.0
    local scale = (smin == smax) and smin or (smin + math.random() * (smax - smin))
    for _,d in ipairs(m:GetDescendants()) do
        if d:IsA("BasePart") then
            -- Uniform scale (MeshParts/parts both get size adjusted)
            d.Size = d.Size * scale
            d.Anchored = true
        end
    end
